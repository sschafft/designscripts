...
if (parseInt (app.version) > 5)
	{
	var w = Window.find ("palette", "GREP query manager1");
	if (w != null)
		{
		w.show();
		exit();
...
function show_grep_queries ()
	...
	var w = new Window ("palette", "GREP query manager", undefined, {resizeable: true});
			...
			var chain_panel = buttons.add ("panel"); chain_panel.alignChildren = "fill"; chain_panel.orientation = "column";
				var chain_group = chain_panel.add ("group"); 
					var chain_dd = chain_group.add ("group");
						...
						ChainList.maximumSize.width = 130; ChainList.alignment = ["fill","fill"]; ChainList.id = "ChainList";
...
//~ 						manage_buttons.children[i].preferredSize = [22,20];
...
//~ 						rename_chain.helpTip = "Rename the selected chain.";
						...
						delete_b.helpTip = "Delete the selected query.";
						update_b.helpTip = "Update the query list whenever you add \nor delete queries in the Find/Change dialog.";
	...
	function read_previous ()
		...
		f.open ("r"); var o = eval (f.read()); f.close();
		return o;
		}
	
	print_b.onClick = function () {export_queries(QList)};
...
//~ 		$.writeln (">"+QList.properties.columnWidths)
			...
			function plain_array (list){
				var temp = [];
				if (list !== null)
					for (var i = 0; i < list.length;i++) {temp.push (list[i].text)}
				return temp;
				} // plain_array

		// begin close_b.onClick
		var o = {location: [w.location[0],w.location[1]],
		...
		var f = File (scriptFolder+"grep_query_manager.txt");
		f.open ("w"); f.write (o.toSource()); f.close();
		w.close();
		} // close_b.onClick
...
//~ 	gr_style.enabled = execute_panel.enabled = app.documents.length > 0;
	...
	if (parseInt (app.version) <= 5) {gr_style.enabled = false;}
	update_b.onClick = function () {UpdateList (QList, QList.selection[0].text);}
	...
	dd_list.onActivate = function ()
		{
		function insert_item (dd, str) {if (dd.find(str) === null) dd.add("item", str);}
		...
		if (IsText()) {insert_item (dd_list, "Story"); insert_item (dd_list, "Selection")}; 
			else {dd_list.remove (dd_list.find ("Story")); dd_list.remove (dd_list.find ("Selection"))};
		} // dd_list.onActivate
	...
	edit_b.onClick = function () {EditQuery ({button: 2});}
...
//~ 		}
	...
	collect.onClick = function ()
		...
		if (!this.value) {format.value = pagenums.value = this.value;}
	...
	function display (found)
		{
		var frame = app.documents.add().textFrames.add ({geometricBounds: ["36pt", "36pt", "280mm", "190mm"]});
		frame.textFramePreferences.textColumnCount = 2;
		if (format.value == true)
			{
			var pnum;
			for (var i = 0; i < found.length; i++)
				{
				pagenums.value ? pnum = " " + find_page (found[i]).name : pnum = "";
				found[i].duplicate (LocationOptions.after, frame.insertionPoints[-1]);
				frame.insertionPoints[-1].contents = pnum + "\r";
				}
			}
		else
			{
			frame.contents = FindPageNums (found, pagenums);
			}
		} // display
	...
	function with_pnums (found)
		...
		function unique(/*str[]*/ arr)
			{ // This is the version that Marc Autret posted in the scripting forum
			var o={}, r=[], n = arr.length, i;
			for( i=0 ; i<n ; ++i ) {o[arr[i]] = null;}
			for( i in o ) {r.push(i);}
			return r;
			} // unique
...
// Tools ==========================================================================================
	...
	w.onChange = function (){ChainList.active = true;}
...
// End Tools ==========================================================================================
	...
	w.onShow = function ()
		...
		try {collect.value = HISTORY.collect_items} catch (_){};
//~ 		execute_panel.enabled = app.documents.length > 0;
		try {help_b.value = HISTORY.helptips;} catch (_) {help_b.value = true;}
...
function export_queries (QList)
	...
	var next_frame, gb = story.textContainers[0].geometricBounds;
	doc.viewPreferences.rulerOrigin = RulerOrigin.pageOrigin;
	while (story.overflows)
		{
		doc.pages.add (LocationOptions.atEnd);
...
function create_document ()
	{
		...
		function create_nested_styles (pstyle)
			{
			var comment, cstyles = pstyle.parent.characterStyles;			
